...
CODIS_PROXY_BIN=$CODIS_BIN_DIR/proxy
CODIS_PROXY_PID_FILE=$CODIS_BIN_DIR/proxy.pid

CODIS_PROXY_LOG_FILE=$CODIS_LOG_DIR/proxy.log
CODIS_PROXY_DAEMON_FILE=$CODIS_LOG_DIR/proxy.out
...
stop)
    ...
    then
      echo "no proxy to stop (could not find file $CODIS_PROXY_PID_FILE)"
    else
