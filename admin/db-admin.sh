...
CODIS_DASHBOARD_BIN=$CODIS_BIN_DIR/dashboard
CODIS_ADMIN_TOOL_BIN=$CODIS_BIN_DIR/config
CODIS_DASHBOARD_PID_FILE=$CODIS_BIN_DIR/dashboard.pid

CODIS_DASHBOARD_LOG_FILE=$CODIS_LOG_DIR/dashboard.log
CODIS_DASHBOARD_DAEMON_FILE=$CODIS_LOG_DIR/dashboard.out
...
stop)
    ...
    then
      echo "no dashboard to stop (could not find file $CODIS_DASHBOARD_PID_FILE)"
    else
...
restart)
    ...
    ;;
remove-lock)
    $CODIS_ADMIN_TOOL_BIN -v --remove-lock --product=stable_1 --zookeeper=127.0.0.1:2181
    ;;
*)
