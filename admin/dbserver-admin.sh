...
CODIS_SERVER_BIN=$CODIS_BIN_DIR/dbserver
CODIS_SERVER_PID_FILE=/tmp/redis_6379.pid

CODIS_SERVER_LOG_FILE=/tmp/redis_6379.log
CODIS_SERVER_DAEMON_FILE=$CODIS_LOG_DIR/dbserver.out
...
start)
    ...
    ;;
stop)
    echo "stopping dbserver ... "
    if [ ! -f "$CODIS_SERVER_PID_FILE" ]
    then
      echo "no dbserver to stop (could not find file $CODIS_SERVER_PID_FILE)"
    else
