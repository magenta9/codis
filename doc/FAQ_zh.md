...
2) 原来使用 Redis 的用户:
如果你使用了[doc/unsupported_cmds](https://github.com/magenta9/codis/blob/master/doc/unsupported_cmds.md)中提到的命令，是无法直接迁移到 Codis 上的. 你需要修改你的代码, 用其他的方式实现.
...
见Readme中的[Benchmark一节](https://github.com/magenta9/codis#performance-benchmark)。
...
但是需要注意：请使用codis2.0.10或更新的版本，旧版对etcd的支持有一些问题；由于etcd在2.2引入了一个与旧版不兼容的坑爹改动，导致如果使用etcd的版本>=2.2.0，暂时需要手动改一处代码，详情见[相关issue](https://github.com/magenta9/codis/issues/488)。

### 现有redis集群上有上T的数据，如何迁移到Codis上来？

为了提高 Codis 推广和部署上的效率，我们为数据迁移提供了一个叫做 [redis-port](https://github.com/magenta9/redis-port) 的命令行工具，它能够：
...
+ redis-port 本质是以 slave 的形式挂载到现有 redis 服务上去的
	...
	2. redis-port 分析 RDB 文件，并拆分成 key-value 对，通过 [slotsrestore](https://github.com/magenta9/codis/blob/master/doc/redis_change_zh.md#slotsrestore-key1-ttl1-val1-key2-ttl2-val2-) 指令发给 codis
...
说明没有正确的设置go项目路径导致生成的文件找不到。见[安装教程](https://github.com/magenta9/codis/blob/master/doc/tutorial_zh.md#build-codis-proxy--codis-config)来正确配置环境变量并用正确的方式下载代码。
...
这时如果client用了我们修改的Jedis, [Jodis](https://github.com/magenta9/jodis)，是会监控到zk节点上proxy少了一个从而自动避开请求这个proxy以保证客户端业务的可用性。如果用非Java语言可以根据Jodis代码DIY一个监听zk的客户端。
另外，如果需要异步请求，可以使用我们基于Netty开发的[Nedis](https://github.com/magenta9/nedis)。
